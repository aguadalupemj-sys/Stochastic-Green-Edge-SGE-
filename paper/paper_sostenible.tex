...
carbono} asociados a la inspecci\'on de tr\'afico, como alternativa cuantificablemente
m\'as sostenible que la inspecci\'on profunda continua.

\subsection{Objetivos espec\'ificos}
\begin{enumerate}\itemsep2pt
  \item \textbf{(OE-1) L\'ogica estoc\'astica y de teor\'ia de juegos.} Formalizar el
        juego Defensor--Atacante, derivar el \emph{equilibrio de Nash} en estrategias
        mixtas y materializar una pol\'itica de inspecci\'on intermitente y
        adaptativa, reproducible bajo semilla fija (\emph{seed-7}).
  \item \textbf{(OE-2) Capa de seguridad en APIs.} Construir el m\'odulo de
        filtrado/inspecci\'on sobre HTTP/REST, con inspecci\'on de \emph{payloads} y
        \emph{prompts} para detectar \emph{prompt injection} y abuso de peticiones.
  \item \textbf{(OE-3) Medici\'on y reducci\'on del consumo energ\'etico.}
        Instrumentar la telemetr\'ia de recursos y su conversi\'on a huella de carbono
        (gCO\textsubscript{2}eq), integrando el coste energ\'etico en el \emph{payoff}
        del Defensor.
  \item \textbf{(OE-4) Validaci\'on y \emph{benchmarking}.} Comparar SGE frente a
        m\'etodos convencionales (inspecci\'on continua y muestreo uniforme fijo),
        reportando eficiencia energ\'etica \emph{vs.} tasa de detecci\'on (frontera de
        Pareto) bajo \emph{seed-7}.
\end{enumerate}

\subsection{Alcance (\emph{scope})}
Esta fase persigue un \textbf{prototipo funcional validado experimentalmente}.
\textbf{Dentro del alcance:} el algoritmo estoc\'astico (Nash) reproducible
(\emph{seed-7}); el m\'odulo de filtrado de peticiones/APIs sobre HTTP/REST; la
inspecci\'on de \emph{payloads}/\emph{prompts}; las m\'etricas de consumo energ\'etico
y su traducci\'on a gCO\textsubscript{2}eq; y el prototipo funcional con
\emph{benchmarking} reproducible. \textbf{Fuera del alcance} en esta fase quedan el
despliegue en cl\'usteres multirregi\'on a gran escala, las interfaces gr\'aficas
complejas de administraci\'on, el soporte para protocolos industriales propietarios no
HTTP/REST (gRPC, MQTT, OPC-UA, Modbus) y la certificaci\'on/\emph{hardening}
productivo. \textbf{Entregables clave:} artefactos de c\'odigo (motor de decisi\'on
estoc\'astico y proxy/gateway de inspecci\'on), documentaci\'on t\'ecnica y modelo
matem\'atico de la estrategia de inspecci\'on, y reporte de \emph{benchmarks}
(eficiencia energ\'etica \emph{vs.} tasa de detecci\'on de amenazas).
